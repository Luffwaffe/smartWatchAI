 ...
 */
extern "C" void pmu_print_status(void)
{
    ...
    if (PMU.isBatteryConnect()) {
        ...
        ESP_LOGI(TAG, "  Voltage: %u mV", PMU.getBatteryVoltage());
        ESP_LOGI(TAG, "  Charging: %s", PMU.isCharging() ? "Yes" : "No");
        if (PMU.isCharging()) {
            ESP_LOGI(TAG, "  Charge Current: %u mA", PMU.getBatteryChargeCurrent());
        }
    } else {
